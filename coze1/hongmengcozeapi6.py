...
import processor  # 关键：导入processor模块，使其执行注册代码
from common import get_callback  # 使用getter函数获取回调
...
def receive_and_process():
    try:
        ...
        print(f"\n===== 前端传入的数据 =====")
        print(input_content)
        # 关键：调用另一个文件的处理函数（如果已注册）
        callback = get_callback()  # 获取注册的回调
        if callback:
            callback(input_content)  # 调用processor.py中的handle_input_content
        else:
            print("未找到注册的回调函数，请检查processor.py是否被正确导入")
